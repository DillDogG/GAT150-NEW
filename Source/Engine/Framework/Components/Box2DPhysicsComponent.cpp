...
namespace kiko {
	...
	void Box2DPhysicsComponent::ApplyForce(const vec2& force) { m_body->ApplyForceToCenter(VEC2_TO_B2VEC2(force), true); }

	void Box2DPhysicsComponent::ApplyTorque(float torque) { m_body->ApplyTorque(torque, true); }

	void Box2DPhysicsComponent::SetVelocity(const vec2& velocity) { m_body->SetLinearVelocity(VEC2_TO_B2VEC2(velocity)); }
