...
namespace kiko {
	...
	void kiko::Renderer::DrawTexture(Texture* texture, const Transform& transform) {
		...
		dest.y = (int)(position.y - (size.x * 0.5f));
		dest.w = (int)size.x;
		dest.h = (int)size.y;
	...
	void Renderer::DrawTexture(Texture* texture, const Rect& source, const Transform& transform, const vec2& origin, bool flipH) {
		...
		dest.y = (int)(position.y - (size.x * origin.y));
		dest.w = (int)size.x;
		dest.h = (int)size.y;
